...
export default function FeaturedWinesSection({ isAdult = true }: FeaturedWinesSectionProps) {
  ...
  useEffect(() => {
    ...
    const maxHeight = isMobile
      ? viewportHeight - 40 // En móvil: casi pantalla completa (dejando espacio arriba/abajo)
    ...
    const centeredTop = isMobile ? 20 : (viewportHeight - maxHeight) / 2
...
function WineCardExpanded({
  ...
  return (
    <div ref={contentRef} className="flex flex-col md:flex-row h-full overflow-hidden" style={{ opacity: isVisible ? 1 : 0 }}>
      ...
      <div className="h-[42%] md:h-full md:w-[42%] relative flex items-center justify-center flex-shrink-0">
        ...
        <img
          ...
          loading="eager"
          decoding="async"
          className="relative z-10 h-[92%] md:h-[85%] w-auto object-contain max-h-[300px] md:max-h-none"
